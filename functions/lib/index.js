...
api.post('/auth/login', async (req, res) => {
    try {
        ...
        if (!valid)
            return res.status(401).json({ error: 'Email ou mot de passe incorrect' });
        const token = jsonwebtoken_1.default.sign({ userId: userDoc.id }, JWT_SECRET, { expiresIn: '30d' });
        const userSafe = {
            id: userDoc.id,
            email: user.email,
...
api.post('/internal/build/:buildId/upload', builderAuthMiddleware, express_1.default.raw({ type: '*/*', limit: '100mb' }), async (req, res) => {
    try {
        const buildId = req.params.buildId;
        const doc = await db.collection('builds').doc(buildId).get();
        if (!doc.exists)
...
api.post('/notifications/webhook', async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(400).json({ error: 'userId required in webhook payload' });
...
api.get('/download-latest', async (req, res) => {
    try {
        const appId = req.query.appId;
        if (!appId)
            return res.status(400).send('appId required');
        const snap = await db.collection('builds')
...
api.get('/apps/check-update', async (req, res) => {
    try {
        ...
        const versionCode = parseInt(req.query.versionCode);
        if (!packageName)
        ...
        for (const cssMatch of cssMatches) {
            try {
                ...
                if (!cssUrl.startsWith('http'))
                    cssUrl = new URL(cssUrl, new URL(targetUrl).origin).toString();
                const cssRes = await (0, node_fetch_1.default)(cssUrl, { timeout: 5000 });
                if (cssRes.ok) {
...
exports.api = (0, https_1.onRequest)({ cors: true, timeoutSeconds: 540, memory: '1GiB', region: 'us-central1' }, app);
